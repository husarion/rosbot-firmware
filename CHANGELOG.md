...
- Release pipeline opens auto-PRs against `husarion/rosbot_ros` (`jazzy-mavlink-v2`) and `husarion/rosbot-snap` (`jazzy-mavlink`) after every dispatched release, bumping the firmware pin in `rosbot_hardware.repos`, swapping bundled `.bin` artefacts (prefix-match, also cleans up legacy `_mavlink-` variants), updating `flash_firmware` bundled paths, and bumping the expected-firmware tag in mavlink launch files. `rosbot_ros` PRs carry the `auto-firmware-bump` label so the companion auto-merge workflow squash-merges them once CI is green; snap PRs stay for human review. New jobs `bump-rosbot-ros` and `bump-rosbot-snap` live at the bottom of [release.yaml](.github/workflows/release.yaml).
...
- Required repo secrets in `rosbot-firmware`: `HUSARION_BOT_APP_ID`, `HUSARION_BOT_PRIVATE_KEY` (PEM private key). GitHub App must be installed on `husarion/rosbot_ros` and `husarion/rosbot-snap` with `contents:write` + `pull-requests:write`. App tokens are short-lived (1h), owner-controlled, no annual rotation.
