...
#ifdef TIM9
    else if (t == TIM9)  { __HAL_RCC_TIM9_CLK_ENABLE();  }
#endif
#ifdef TIM10
    else if (t == TIM10) { __HAL_RCC_TIM10_CLK_ENABLE(); }
#endif
#ifdef TIM11
    else if (t == TIM11) { __HAL_RCC_TIM11_CLK_ENABLE(); }
#endif
#ifdef TIM12
...
void FanController::configureChannel() {
    ...
    oc.OCNPolarity  = config_.complementary
...
void FanController::update(float temp) {
    ...
    if (temp <= config_.temp_low) {
        ...
        applyDuty(static_cast<uint8_t>((normalized * (100u - config_.duty_min) + config_.duty_min)));
        running_ = true;
    }
}
