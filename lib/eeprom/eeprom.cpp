...
EepromStatus Eeprom::writePage(uint8_t block, uint8_t addr,
...
EepromStatus Eeprom::readPage(uint8_t block, uint8_t addr,
                              uint8_t* data, uint8_t size)
{
    ...
    return (i2c_bus_.readBytes(data, size) == size)
        ? EepromStatus::Ok
...
// ── BoardRevision ───────────────────────────────────────
BoardRevision::BoardRevision(const BoardRevisionConfig cfg) 
    : cfg_(cfg) {}
...
void BoardRevision::init() {
    ...
    for (uint8_t attempt = 0; attempt < cfg_.retry_count; ++attempt) {
        if (cfg_.eeprom.readPage(cfg_.block, cfg_.addr,
                reinterpret_cast<uint8_t*>(buf), cfg_.max_length)
            != EepromStatus::Ok) {
            continue;
        }
